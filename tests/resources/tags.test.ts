...
import { FrontTags } from "../../src/index";
import { createMockClient, jsonResponse } from "../helpers/setup";

describe("tags", () => {
  test("tags.get returns a hydrated FrontTags target and sends GET /tags/{id}", async () => {
    ...
    expect(tag).toBeInstanceOf(FrontTags);
  ...
  test("tags.update sends PATCH without fetching the tag first", async () => {
    ...
    await front.tags.update("tag_abc123", { name: "renamed" });
  ...
  test("FrontTags.save persists property mutations after get", async () => {
    const { front, requests } = createMockClient((req) => {
      const { url } = req;
